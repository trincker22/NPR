...
classify_immigration_frame <- function(text) {
  Sys.sleep(15)  # requests per minute isnt an issue, BUT this prompt is large and requires a lot of tokens, so it will hit the token per minute limit even though there are unlimited tokens overall. 15 is most reliable for me but ofc extremely slow
...
write_rds(immigration, "immigration.rds")

# Calculate precision & recall
immigration$gemini_label <- gsub("/", ".", immigration$gemini_label)
